...
class UserRegistrationForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['username', 'email', 'first_name','last_name','enrollment_number', 'semester', 'department', 'password1', 'password2']
        widgets = {
...
class CollegeAlertForm(forms.ModelForm):
    class Meta:
        model = CollegeAlert
        fields = ['title', 'alert_type', 'description', 'expires_at', 'is_active']
        widgets = {
            'description': forms.Textarea(attrs={
                'rows': 5,
                'class': 'form-control',
